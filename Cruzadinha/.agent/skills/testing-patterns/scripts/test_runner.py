...
def run_tests(cmd: List[str], cwd: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        out_str = proc.stdout if isinstance(proc.stdout, str) else str(proc.stdout or "")
        err_str = proc.stderr if isinstance(proc.stderr, str) else str(proc.stderr or "")
        result["output"] = out_str[:3000]  # type: ignore
        result["error"] = err_str[:500]  # type: ignore
